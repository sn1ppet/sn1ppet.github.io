...
var canIUse = (function () {
    ...
    */
    ...
    //  Customise HTML here
    var TMPL_TITLE = '<h3>{title}</h3>', // feature title {title}
        TMPL_STATUS = '<p class="status">{status}</p>', // feature status (W3C Recommendation) {status}
        TMPL_USAGE = '<p>Global support: <strong>{usage_perc}%</strong></p>', // feature status (W3C Recommendation) {status}
        ...
        TMPL_PREFIX_NOTE = '<p>Prefix required.</p>',
        ...
        TMPL_FOOTER = '<p class="stats">See <a href="http://caniuse.com/#feat={feature}" target="_blank">caniuse.com</a></p>',
    ...
    function generateHtml(results) {
        ...
        resultHtml += TMPL_USAGE.replace('{usage_perc}', results.code.usage_perc_y);
